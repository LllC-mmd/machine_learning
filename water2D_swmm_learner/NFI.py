...
class NumpyFunctionInterface(ParamGroupsManager):
    ...
    def __init__(self, params, forward, *, isfrozen=False, x_proj=None, grad_proj=None, always_refresh=True, **kw):
    ...
    def add_param_group(self, param_group):
        ...
        for _,p in param_group_tmp['params'].items():
            ...
            if not p.requires_grad:
                raise ValueError("managing a Variable that does not require gradients")
        self.options_refresh()
    ...
    def _flat_grad(self):
        ...
        for isfrozen, p in self.params_with_info('isfrozen'):
            if isfrozen or p.grad is None:
                ...
                view = p.grad.data.view(-1).cpu()   # get gradients stored in p.grad
            views.append(view)
        # for view in views:
            # print(view)
        # grad_num += 1
        # print("Gradient eval: %d" % grad_num)
        return torch.cat(views, 0).numpy()
    ...
    def fprime(self, x, always_double=True, *args, **kw):
        ...
        if self._need_backward:
            ...
            self._grad_cache = self._flat_grad()  # backpropagation
